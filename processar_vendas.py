...
st.markdown("""
    ...
    /* BOTÕES DAS CATEGORIAS (FILTROS) E TRAVA DE QUEBRA DE LINHA */
    ...
    /* AQUI ESTÁ A MÁGICA QUE IMPEDE O TEXTO DE QUEBRAR (BOMBO/NIERE NUNCA MAIS) */
...
# --- PROTEÇÃO DE PROPORÇÃO PARA O LOGIN ---
if not st.session_state.get("authentication_status"):
    st.markdown("""
        <style>
        .block-container { max-width: 450px !important; padding-top: 10vh !important; }
        </style>
        """, unsafe_allow_html=True)
...
authenticator = stauth.Authenticate(credentials_dict, "canada_bi_v30", "auth_key_v30", expiry_days=30)
...
if st.session_state.get("authentication_status"):
    ...
    # --- LOGO DO MENU LATERAL AINDA MAIS ELEGANTE ---
    if os.path.exists("logo.png"):
    ...
    if pagina == "Análise de Relatório":
        ...
        if date.today() > trial_end or (config_usuarios[user_logado]["quota"] <= 0 and user_logado != "madson"):
            ...
            if st.session_state.arquivo_carregado is None:
                ...
                # REBALANCEAMENTO DE COLUNAS PARA EVITAR QUEBRAS E ESPREMER O BOTÃO
                col_filtros, col_total, col_detalhes = st.columns([3.2, 3, 4.8], gap="large")
                selecionadas = []
                ...
                with col_filtros:
                    ...
                    for cat in categorias_pdf:
                        ...
                        # Ajuste preciso interno da coluna de filtros
                        c_chk, c_btn, c_val = st.columns([0.8, 5.5, 3.5])
                        with c_chk:
    ...
    st.info("Plataforma Restrita. Insira as tuas credenciais para prosseguir.")import streamlit as st
import streamlit_authenticator as stauth
import pdfplumber
import re
import pandas as pd
